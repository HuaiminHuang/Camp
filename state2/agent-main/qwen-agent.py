...
load_dotenv(r"C:\Users\hhm18\Desktop\course\apikey.env")
...
def llm(query, history=[], user_stop_words=[]):
    ...
    # 准备请求数据
    payload = {
        "model": "deepseek-chat",  # DeepSeek 主要模型
        "messages": messages,
        "temperature": 0.1,  # 低温度确保输出更确定性
        "max_tokens": 1024,
    }
    
    # 处理停止词（DeepSeek API 支持 stop 列表）
    if user_stop_words:
        payload["stop"] = user_stop_words
    ...
    try:
        ...
        return f"DeepSeek API response parsing error: {str(e)}"
